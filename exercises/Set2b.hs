...
countdown n = "Ready! " ++ countdown_helper n ++ "Liftoff!"
  where countdown_helper n  
          | n == 0 = ""
          | otherwise = show n ++ "... " ++ countdown_helper (n-1)
...
smallestDivisor n = head [k |  k <- [2..n], n `mod` k == 0]
...
biggestPrimeAtMost n = head [k | k <- [n, n-1..2], isPrime k]
