...
sums :: Int -> [Int]
sums n = sumsHelper n 1 []

sumsHelper :: Int -> Int -> [Int] -> [Int]
sumsHelper n k ans = if n == k then [k * (k + 1) `div` 2] else k * (k + 1) `div` 2 : sumsHelper n (k + 1) ans
...
sorted [x] = True
sorted [x, y] = x <= y 
sorted (x : y : xs) = sorted [x, y] && sorted (y : xs)
...
sumsOfHelper [] cumsum = []
sumsOfHelper [x] cumsum = [x + cumsum]
sumsOfHelper (x : xs) cumsum = (x + cumsum) : sumsOfHelper xs (cumsum + x)
