...
whileRight check a = case check a of 
  Left b -> b
...
joinToLength k xs = [a ++ b | a <- xs, b <- xs, length (a ++ b) == k]
...
sumRights xs = sumNums(map (either (const 0) id) xs)
...
interpreter commands = interpreterHelp 0 0 commands []

interpreterHelp :: Int -> Int -> [String] -> [String] -> [String]
interpreterHelp x y [] res = res
interpreterHelp x y ("up" : commands) res = interpreterHelp x (y + 1) commands res
interpreterHelp x y ("down" : commands) res = interpreterHelp x (y - 1) commands res
interpreterHelp x y ("left" : commands) res = interpreterHelp (x - 1) y commands res
interpreterHelp x y ("right" : commands) res = interpreterHelp (x + 1) y commands res
interpreterHelp x y ("printX" : commands) res = interpreterHelp x y commands (res ++ [show x])
interpreterHelp x y ("printY" : commands) res = interpreterHelp x y commands (res ++ [show y])
